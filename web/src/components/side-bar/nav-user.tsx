...
export function NavUser() {
  ...
  const { data: avatarData, isLoading: isPhotoLoading } = useQuery({
    queryKey: ["user-photo", user?.photoId],
    queryFn: () =>
      user?.photoId ? getAttachement({ id: user.photoId }) : null,
              ...
              <div className="flex items-center gap-2 px-1 py-1.5 text-left text-sm">
                {isUserLoading ? (
                  ...
                  <>
                    <Avatar className="h-8 w-8 rounded-lg">
                      <AvatarImage
                        src={avatarData?.url ?? "/placeholder.svg"}
                        alt={user?.name}
                      />
                      <AvatarFallback className="rounded-lg">
                        {user?.name ? getInitials(user.name) : "..."}
